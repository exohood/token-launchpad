...
const TOKEN_NAME = 'GeliCoin'
const TOKEN_SYMBOL = 'GCOIN'
const TOKEN_DECIMAL_MULTIPLIER = 10^18
const TOKEN_DECIMALS = 18
const TARGET_USER = 0xd0593b233be4411a236f22b42087345e1137170b
const PAUSED = 0
...
def freezingCount(address _addr): # not payable
  mem[0] = 'EXO' or addr(_addr) << 64
  mem[32] = 3
  idx = stor[sha3(mem[0 len 64])]
  s = 0
  while uint64(idx):
      mem[0] = 'EXO' or addr(_addr) << 64 or uint64(idx)
      mem[32] = 3
      idx = stor3['EXO' or addr(_addr) << 64 or uint64(idx)]
      s = s + 1
      continue 
  return s
...
def getFreezing(address _addr, uint256 _index): # not payable
  ...
  s = 0
  while idx < _index + 1:
      mem[0] = 'EXO' or addr(_addr) << 64 or uint64(s)
      mem[32] = 3
      if stor3['EXO' or addr(_addr) << 64 or uint64(s)]:
          idx = idx + 1
          s = stor3['EXO' or addr(_addr) << 64 or uint64(s)]
          continue 
      return stor3['EXO' or addr(_addr) << 64 or uint64(s)], 0
  return _index + 1 << 198, stor4['EXO' or addr(_addr) << 64 or 64 * _index + 1 % 288230376151711744]
...
def releaseOnce(): # not payable
  require stor3['EXO' or caller << 64]
  require uint64(block.timestamp) > stor3['EXO' or caller << 64]
  stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]] = 0
  require stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]] + actualBalanceOf[caller] >= actualBalanceOf[caller]
  actualBalanceOf[caller] += stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]]
  require stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]] <= freezingBalanceOf[caller]
  freezingBalanceOf[caller] -= stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]]
  if not stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]]:
      stor3['EXO' or caller << 64] = 0
  else:
      stor3['EXO' or caller << 64] = stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]]
      stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]] = 0
  log Released(
        address owner=stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]],
        uint256 amount=caller)
...
def releaseAll(): # not payable
  ...
  s = 0
  while idx < 1:
      mem[0] = 'EXO' or caller << 64 or uint64(s)
      mem[32] = 3
      if stor3['EXO' or caller << 64 or uint64(s)]:
          idx = idx + 1
          s = stor3['EXO' or caller << 64 or uint64(s)]
          continue 
      else:
          return 0
  if not var40002:
  ...
  while idx:
      ...
      if not stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]]:
          ...
          stor3['EXO' or caller << 64] = stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]]
          stor3['EXO' or caller << 64 or stor3['EXO' or caller << 64]] = 0
      mem[96] = stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]]
      log Released(
            address owner=stor4['EXO' or caller << 64 or stor3['EXO' or caller << 64]],
            uint256 amount=caller)
      idx = 0
      u = 0
      while idx < 1:
          mem[0] = 'EXO' or caller << 64 or uint64(u)
          mem[32] = 3
          if stor3['EXO' or caller << 64 or uint64(u)]:
              idx = idx + 1
              u = stor3['EXO' or caller << 64 or uint64(u)]
              continue 
          return (s + t)
      mem[0] = 'EXO' or caller << 64 or 64
      mem[32] = 4
      idx = 1
      s = stor4['EXO' or caller << 64 or 64]
      t = 64
      t = s + t
      continue 
  return t
...
def freezeTo(address _to, uint256 _amount, uint64 _until): # not payable
  ...
  require _amount + stor4['EXO' or addr(_to) << 64 or _until] >= stor4['EXO' or addr(_to) << 64 or _until]
  stor4['EXO' or addr(_to) << 64 or _until] += _amount
  require _amount + freezingBalanceOf[addr(_to)] >= freezingBalanceOf[addr(_to)]
  freezingBalanceOf[addr(_to)] += _amount
  require _until > block.timestamp
  if not stor3['EXO' or addr(_to) << 64]:
      stor3['EXO' or addr(_to) << 64] = _until
  else:
      if not uint64(var32003):
          if _until != uint64(var32003):
              if uint64(var32003):
                  stor3['EXO' or addr(_to) << 64 or _until] = uint64(var32003)
          ...
          while var35001:
              ...
              if stor3[var35003]:
                  ...
                  s = uint64(var35004)
                  s = 'EXO' or addr(_to) << 64 or stor3[var35003]
                  s = stor3[var35003]
                  s = var35003
                  continue 
              if _until != stor3[var35003]:
                  if stor3[var35003]:
                      stor3['EXO' or addr(_to) << 64 or _until] = stor3[var35003]
          ...
          if _until != uint64(var35004):
              if uint64(var35004):
                  stor3['EXO' or addr(_to) << 64 or _until] = uint64(var35004)
